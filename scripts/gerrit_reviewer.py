...
def build_inline_review_prompt(
    ...
    # annotated_diff 우선 사용 (없으면 기존 방식으로 fallback)
    if diff.annotated_diff:
        diff_text       = diff.annotated_diff
        valid_new_lines = diff.valid_new_lines
    else:
        diff_text       = diff.diff_content
        valid_new_lines = _extract_added_lines(diff.diff_content)

    valid_lines_hint = (
        f"\n코멘트 가능한 라인 번호 목록 (RIGHT/REVISION side): {valid_new_lines[:50]}"
        if valid_new_lines else ""
    )
...
{focus_str}{valid_lines_hint}

■ diff 읽는 법 (라인 번호 안내):
  [A  N] +코드  → N번 라인에 추가된 줄  → side="REVISION", line=N 으로 코멘트
  [D  N] -코드  → N번 라인에서 삭제된 줄 → side="PARENT",   line=N 으로 코멘트
  [   N]  코드  → N번 라인 컨텍스트(변경 없음)
  [] 형식이 없으면 기존 unified diff 형식: + 추가 / - 삭제 / 공백 컨텍스트
...
{{
  ...
  "inline_comments": [
    {{
      "line": <위 diff에서 [A N] 또는 [D N]의 N 값 (정수)>,
      "side": "<REVISION(추가줄) 또는 PARENT(삭제줄)>",
      "severity": "<CRITICAL|MAJOR|MINOR|INFO>",
...
- line 은 diff 의 [A N] 또는 [D N] 에서 읽은 N 값을 그대로 사용 (절대 추측 금지)
- 추가된 줄([A N])에 코멘트: side="REVISION" / 삭제된 줄([D N])에 코멘트: side="PARENT"
- message 는 300자 이내로 간결하게 (줄바꿈은 \\n 사용)

■ severity 판단 기준 (반드시 이 기준에 따라 일관되게 분류):
  CRITICAL : 즉시 수정 필요. 아래 중 하나라도 해당하면 CRITICAL
             · SQL Injection / XSS / 인증 우회 / 평문 비밀번호 비교·저장
             · NullPointerException / ArrayIndexOutOfBounds 등 확정적 런타임 크래시
             · 데이터 손실·오염 가능성 (트랜잭션 누락, 파일 덮어쓰기 등)
  MAJOR    : 운영 환경에서 장애·오작동을 일으킬 수 있는 로직 결함
             · thread-safe 하지 않은 공유 상태 (Race Condition)
             · 잘못된 예외 처리 (catch 후 무시, 범위가 너무 넓은 catch)
             · N+1 쿼리, 무한 루프, 메모리 누수 가능성
             · 비즈니스 로직 오류 (조건 반전, off-by-one 등)
  MINOR    : 코드 품질·유지보수성 문제. 당장 장애는 아니지만 개선 필요
             · 의미 없는 변수명 / 과도하게 긴 함수 / 중복 코드
             · 하드코딩된 매직 넘버 / 상수 미분리
             · 주석 없는 복잡한 알고리즘
  INFO     : 선택적 개선 제안. 무시해도 되는 수준
             · 더 나은 API / 라이브러리 / 패턴 제안
             · 스타일 가이드 권장사항 (강제가 아닌 경우)

■ 좋은 message 예시:
  CRITICAL: "평문 비밀번호 비교: user.getPassword().equals(pwd)는 DB에 평문 저장을 전제합니다.\nBCryptPasswordEncoder.matches(rawPwd, encodedPwd)로 교체하세요."
  MAJOR:    "HashMap은 thread-safe하지 않습니다. 동시 요청 시 ConcurrentModificationException 발생 가능.\nConcurrentHashMap 또는 Collections.synchronizedMap()으로 교체하세요."
  MINOR:    "변수명 'd'는 의미를 알 수 없습니다. 'diffContent' 또는 'fileDiff'로 변경하면 가독성이 향상됩니다."
  INFO:     "Optional.get() 대신 Optional.orElseThrow()를 사용하면 예외 메시지를 명시할 수 있습니다."

■ 평가 일관성 원칙:
  - 동일한 패턴의 문제는 항상 동일한 severity로 분류
  - 불확실한 경우 한 단계 낮은 severity 선택 (MAJOR 불확실 → MINOR)
  - 추측성 코멘트 금지 ("아마도 문제가 있을 수 있다" 등의 표현 사용 금지)
  - 실제 코드에서 확인 가능한 문제만 작성
...
# Gerrit CommentInput.side 허용값: "REVISION"(새 파일) / "PARENT"(기존 파일)
# AI 가 반환하는 "RIGHT"/"LEFT" 는 Gerrit API 스펙에 없으므로 변환 필요
_SIDE_MAP = {
    "RIGHT":    "REVISION",
    "LEFT":     "PARENT",
    "REVISION": "REVISION",
...
def _snap_to_valid_line(line: int, valid_new_lines: list[int]) -> tuple[int, bool]:
    """
    AI 가 반환한 라인 번호가 실제 추가된 줄 목록에 없으면
    가장 가까운 유효 라인으로 보정합니다.

    Returns:
        (보정된 라인 번호, 보정 여부)
    """
    if not valid_new_lines or line in valid_new_lines:
        return line, False
    nearest = min(valid_new_lines, key=lambda x: abs(x - line))
    return nearest, True
...
def _build_result(
    data:            dict,
    filename:        str,
    valid_new_lines: list[int] | None = None,   # RIGHT(REVISION) 코멘트 가능 라인 목록
) -> tuple[str, list[dict]]:
    """
    파싱된 JSON dict에서 (file_summary, inline_comments)를 구성합니다.

    valid_new_lines 가 제공되면:
      - REVISION(RIGHT) side 코멘트의 라인 번호를 검증하고,
        실제 추가된 라인 목록에 없으면 가장 가까운 유효 라인으로 자동 보정합니다.
    ...
    for c in raw_comments:
        try:
            ...
            # side 값 정규화: AI 반환 "RIGHT"/"LEFT" → Gerrit API "REVISION"/"PARENT"
            raw_side = c.get("side", "RIGHT").upper()
            side     = _SIDE_MAP.get(raw_side, "REVISION")
...
def parse_inline_comments(
    ai_response:     str,
    filename:        str,
    valid_new_lines: list[int] | None = None,   # 라인 번호 검증용 (get_file_diff 에서 전달)
) -> tuple[str, list[dict]]:
    """
    ...
    valid_new_lines 가 제공되면 _build_result() 에서 라인 번호 검증/보정을 수행합니다.

    Returns:
        file_summary   : 파일 전체 요약 텍스트 (절대 JSON 코드블록이 포함되지 않음)
        inline_comments: [{"line": N, "side": "REVISION"/"PARENT", "severity": "...", "message": "..."}]
    """
    import re
    ...
    try:
        ...
        return _build_result(data, filename, valid_new_lines)
...
■ severity 판단 기준 (반드시 이 기준에 따라 일관되게 분류):
  ...
  MAJOR    : 운영 환경에서 장애·오작동을 일으킬 수 있는 로직 결함
             · thread-safe 하지 않은 공유 상태 (Race Condition)
             · 잘못된 예외 처리 (catch 후 무시, 범위 과대)
             · N+1 쿼리 / 무한 루프 / 메모리 누수 가능성
             · 비즈니스 로직 오류 (조건 반전, off-by-one 등)
  MINOR    : 코드 품질·유지보수성 문제. 당장 장애는 아니지만 개선 필요
             · 의미 없는 변수명 / 과도하게 긴 함수 / 중복 코드
             · 하드코딩된 매직 넘버 / 주석 없는 복잡한 알고리즘
  INFO     : 선택적 개선 제안 (무시해도 무방한 수준)

■ 평가 일관성 원칙:
  - 동일한 패턴의 문제는 항상 동일한 severity로 분류
  - 불확실한 경우 한 단계 낮은 severity 선택 (MAJOR 불확실 → MINOR)
  - 추측성 코멘트 금지 ("아마도 문제가 있을 수 있다" 등 표현 금지)
  - 실제 코드에서 확인 가능한 문제만 작성
- 언어: {language}
            ...
            if item and isinstance(item, dict):
                try:
                    results.append(
                        _build_result(
                            item, diff.filename,
                            valid_new_lines=diff.valid_new_lines or None,
                        )
                    )
                    continue
...
def run_review(
    ...
    ai = create_ai(
        ...
        temperature=ai_cfg.get("temperature"),   # None이면 BaseAI.DEFAULT_TEMPERATURE(0.2) 사용
    )
    logger.info("  AI 모델: %s (temperature=%.2f)", ai.model, ai.temperature)
    ...
    def _review_single(diff):
        ...
        file_summary, inline_comments = parse_inline_comments(
            single_resp.answer, diff.filename,
            valid_new_lines=diff.valid_new_lines or None,
        )
        logger.debug(
